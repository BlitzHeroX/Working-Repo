...
int main (){
	...
	while (cin.fail() || sen_char != 'C' && sen_char != 'c' && sen_char != 'Q' && sen_char !=  'q'){
		if (cin.fail()){
			cin.clear();																					// emptying of input variable //
        	string dummy;																					//declaration of dummy string to store failed input//
        	cin >> dummy;																					//storing of failed input in dummy string //
	        cout << "Please enter a valid key (Either C to continue or Q to exit). " ;
	        cin >> sen_char;
			}
		else if (sen_char != 'C' && sen_char != 'c' && sen_char != 'Q' && sen_char !=  'q'){
			cin.clear();																					// this section compensates for incorrect characters enterred. //
			cout << "Please enter a valid key (Either C to continue or Q to exit). " ;
	        cin >> sen_char;	
		}
	}
	...
	if (sen_char == 'C' || sen_char == 'c'){										//Program runs if C or c is entered //
		run = true;
	}
	else if (sen_char == 'Q' || sen_char ==  'q'){									//Program exits if Q or q is entered //
		run = false;
	}
	...
	while (run){																								
		...
		while (cin.fail() || age < MIN_AGE || age > MAX_AGE){ 														//checks to see if there is failed input or age is not within age range //
			if (cin.fail()){																						//applied consistently through all variable for validation //
				cin.clear();
				string dummy;
				cin >> dummy;
		...
		cout << "\t1. Sedentary (little or no exercise) " <<endl;													//use of tabbed list for aesthetics //
		...
		cout <<endl;
		cout << "Enter a value from 1-3 from the list. " ;
		cin >> goal;
		...
		if (gender == 'M' || gender == 'm'){
			bmr = M_BMR_CONSTANT + (M_WEIGHT_MULTIPLIER * weight) + (M_HEIGHT_MULTIPLIER * height) - (M_AGE_MULTIPLIER * age);		//storing male calculation in bmr variable //
		}
		else if (gender == 'F' || gender == 'f'){
			bmr = F_BMR_CONSTANT + (F_WEIGHT_MULTIPLIER * weight) + (F_HEIGHT_MULTIPLIER * height) - (F_AGE_MULTIPLIER * age);		//storing female calculation in bmr variable //	
		}
		...
		if (gender == 'M' || gender == 'm'){													//report based on male gender //
		...
		else if (gender == 'F' || gender == 'f'){								//report based on female gender //
		...
		system ("PAUSE");																					// to hold the output up for viewability //
		cout << "To continue, press C. To exit, press Q. " ; 												//rerun of top portion of whether to continue or exit program//
		cin >> sen_char;
		cout <<endl;
